...
export default function Tracking() {
    var _a = useState('week'), timeRange = _a[0], setTimeRange = _a[1];
    var _b = useQuery({
        queryKey: ["/api/cravings", DEMO_USER_ID],
        initialData: [],
    }), cravingEntries = _b.data, cravingLoading = _b.isLoading;
    var _c = useQuery({
        queryKey: ["/api/cravings", DEMO_USER_ID, "stats"],
        initialData: { average: 0, trend: 0 },
    }), cravingStats = _c.data, statsLoading = _c.isLoading;
    var _d = useQuery({
        queryKey: ["/api/exercise-sessions", DEMO_USER_ID],
        initialData: [],
    }), exerciseSessions = _d.data, sessionsLoading = _d.isLoading;
    var _e = useQuery({
    ...
    }), userStats = _e.data, userStatsLoading = _e.isLoading;
    var _f = useQuery({
        queryKey: ["/api/beck-analyses", DEMO_USER_ID],
        initialData: [],
    }), beckAnalyses = _f.data, beckLoading = _f.isLoading;
    var isLoading = cravingLoading || statsLoading || sessionsLoading || userStatsLoading || beckLoading;
    if (isLoading) {
        return (<>
        <Navigation />
    ...
    var getTriggerColor = function (trigger) {
        var colors = {
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card className="shadow-material" data-testid="card-avg-craving">
            <CardContent className="p-6">
              ...
              <p className={"text-xs ".concat(cravingTrend < 0 ? 'text-success' : 'text-warning')}>
          ...
          <Card className="shadow-material" data-testid="card-best-streak">
            <CardContent className="p-6">
              ...
              <div className="text-2xl font-bold text-foreground" data-testid="text-best-streak">
                {(userStats === null || userStats === void 0 ? void 0 : userStats.longestStreak) || 0}
              </div>
        ...
        <Tabs defaultValue="cravings" className="space-y-6">
          ...
          <TabsContent value="cravings" className="space-y-6">
            <Card className="shadow-material" data-testid="card-craving-history">
              ...
              <CardContent>
                {cravingEntries && cravingEntries.length > 0 ? (<div className="space-y-4">
                    {cravingEntries.slice(0, 10).map(function (entry) { return (<div key={entry.id} className="border border-border rounded-lg p-4" data-testid={"craving-entry-".concat(entry.id)}>
                        ...
                        {entry.triggers && entry.triggers.length > 0 && (<div className="mb-2">
                            <span className="text-xs font-medium text-muted-foreground">Déclencheurs:</span>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {entry.triggers.map(function (trigger, index) { return (<Badge key={index} variant="outline" className={getTriggerColor(trigger)}>
                                  {trigger}
                                </Badge>); })}
                            </div>
                          </div>)}
                        
                        {entry.emotions && entry.emotions.length > 0 && (<div className="mb-2">
                            <span className="text-xs font-medium text-muted-foreground">Émotions:</span>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {entry.emotions.map(function (emotion, index) { return (<Badge key={index} variant="outline" className="bg-blue-100 text-blue-800">
                                  {emotion}
                                </Badge>); })}
                            </div>
                          </div>)}
                        
                        {entry.notes && (<p className="text-sm text-foreground mt-2 italic">"{entry.notes}"</p>)}
                      </div>); })}
                  </div>) : (<div className="text-center py-8" data-testid="empty-cravings">
          ...
          <TabsContent value="exercises" className="space-y-6">
            <Card className="shadow-material" data-testid="card-exercise-history">
              ...
              <CardContent>
                {exerciseSessions && exerciseSessions.length > 0 ? (<div className="space-y-4">
                    {exerciseSessions.slice(0, 10).map(function (session) { return (<div key={session.id} className="border border-border rounded-lg p-4" data-testid={"exercise-session-".concat(session.id)}>
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-sm text-muted-foreground">
                            {formatDate(session.createdAt)}
                          </span>
                          <div className="flex items-center space-x-2">
                            {session.completed && (<Badge className="bg-success text-success-foreground">
                                <span className="material-icons text-sm mr-1">check_circle</span>
                                Complété
                              </Badge>)}
                            {session.duration && (<Badge variant="outline">
                        ...
                        {session.cratingBefore !== null && session.cravingAfter !== null && (<div className="flex items-center space-x-4 text-sm">
                            ...
                            {session.cratingBefore > session.cravingAfter && (<Badge className="bg-success text-success-foreground">
                                -{session.cratingBefore - session.cravingAfter} points
                              </Badge>)}
                          </div>)}
                      </div>); })}
                  </div>) : (<div className="text-center py-8" data-testid="empty-exercises">
          ...
          <TabsContent value="analyses" className="space-y-6">
            <Card className="shadow-material" data-testid="card-beck-history">
              ...
              <CardContent>
                {beckAnalyses && beckAnalyses.length > 0 ? (<div className="space-y-6">
                    {beckAnalyses.slice(0, 5).map(function (analysis) { return (<div key={analysis.id} className="border border-border rounded-lg p-4" data-testid={"beck-analysis-".concat(analysis.id)}>
                        <div className="flex items-center justify-between mb-4">
                          <span className="text-sm text-muted-foreground">
                            {formatDate(analysis.createdAt)}
                          </span>
                          {analysis.emotionIntensity && analysis.newIntensity && (<div className="flex items-center space-x-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          ...
                          {analysis.rationalResponse && (<div className="md:col-span-2">
                  ...
                  </div>) : (<div className="text-center py-8" data-testid="empty-analyses">
                    <span className="material-icons text-6xl text-muted-foreground mb-4">psychology</span>
