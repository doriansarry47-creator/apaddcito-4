...
export default function Education() {
    var _a = useState('addiction'), selectedCategory = _a[0], setSelectedCategory = _a[1];
    var _b = useState([]), completedModules = _b[0], setCompletedModules = _b[1];
    var filteredModules = educationModules.filter(function (module) { return module.category === selectedCategory; });
    var markAsCompleted = function (moduleId) {
      ...
      <main className="container mx-auto px-4 py-6 pb-20 md:pb-6">
        ...
        <section className="mb-8">
          <Card className="shadow-material" data-testid="card-progress-overview">
            ...
            var categoryModules = educationModules.filter(function (m) { return m.category === key; });
            var completed = categoryModules.filter(function (m) { return completedModules.includes(m.id); }).length;
            var progress = (completed / categoryModules.length) * 100;
            return (<div key={key} className="text-center" data-testid={"progress-".concat(key)}>
                      <div className="mb-2">
                      ...
                      <div className="w-full bg-muted rounded-full h-2">
                        <div className="bg-primary h-2 rounded-full transition-all duration-300" style={{ width: "".concat(progress, "%") }}></div>
                      </div>
                    </div>);
        })}
              </div>
          ...
          <Card className="shadow-material" data-testid="card-category-selection">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {Object.entries(categories).map(function (_a) {
            var key = _a[0], label = _a[1];
            return (<Button key={key} variant={selectedCategory === key ? "default" : "outline"} className="h-auto p-4 flex flex-col items-center gap-2" onClick={function () { return setSelectedCategory(key); }} data-testid={"button-category-".concat(key)}>
        ...
        <section>
          ...
          <div className="space-y-6" data-testid="modules-list">
            ...
            var isCompleted = completedModules.includes(module.id);
            return (<Card key={module.id} className="shadow-material" data-testid={"card-module-".concat(module.id)}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <CardTitle className="text-xl">{module.title}</CardTitle>
                          {isCompleted && (<Badge className="bg-success text-success-foreground">
                  ...
                  <CardContent>
                    <Accordion type="single" collapsible className="w-full">
                      {module.content.sections.map(function (section, index) { return (<AccordionItem key={index} value={"section-".concat(index)}>
                          ...
                          <AccordionContent className="space-y-4">
                            ...
                            {section.keyPoints && (<div>
                                <h4 className="font-medium text-foreground mb-2">Points clés :</h4>
                                <ul className="space-y-1">
                                  {section.keyPoints.map(function (point, pointIndex) { return (<li key={pointIndex} className="flex items-start text-sm text-muted-foreground">
                    ...
                    <div className="flex justify-end mt-4">
                      <Button onClick={function () { return markAsCompleted(module.id); }} disabled={isCompleted} className={isCompleted ? "bg-success text-success-foreground" : ""} data-testid={"button-complete-".concat(module.id)}>
                        {isCompleted ? (<>
